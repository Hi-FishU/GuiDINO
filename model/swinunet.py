...
def _to_2tuple(value: int | Tuple[int, int]) -> Tuple[int, int]:
    if isinstance(value, (tuple, list)):
        if len(value) != 2:
            raise ValueError("Expected a tuple of length 2 for image size or patch size.")
...
class SwinTransformerBlock(nn.Module):
    ...
    def flops(self) -> int:
        ...
        flops += int(nW) * self.attn.flops(self.window_size * self.window_size)
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if L != H * W:
            raise ValueError("Input feature has wrong size.")
        if H % 2 != 0 or W % 2 != 0:
            raise ValueError(f"Input resolution ({H}*{W}) is not even.")
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if (H, W) != self.img_size:
            raise ValueError(
                ...
                "Make sure preprocessing resize matches the Swin-Unet img_size."
            )
...
class SwinUnet(SwinTransformerSys):
    """Alias for SwinTransformerSys to match common naming in downstream code."""
